...
#   cidr_block        = var.subnet_cidrs["rds_private_subnets"][0] #"10.0.5.0/24"
...
#   cidr_block        = var.subnet_cidrs["rds_private_subnets"][1] #"10.0.6.0/24"
...
# --- Existing RDS subnet group created in eks cluster---
...
#     filter {
#       name   = "tag:kubernetes.io/cluster/${var.environment}-${var.prefix}-${var.eks_cluster_name}"
#       values = ["owned"]
#     }
# }

# Look up the EKS worker node security group dynamically
data "aws_security_group" "eks_node" {
  filter {
    name   = "tag:kubernetes.io/cluster/${var.environment}-${var.prefix}-${var.eks_cluster_name}"
    values = ["owned"]
...
resource "aws_secretsmanager_secret" "db_link" {
  ...
  description             = "DB link"
  kms_key_id              = aws_kms_key.env_kms.arn
  recovery_window_in_days = 0 # 0 for immediate deletion on destroy / 7 for 7 days retention
  lifecycle {
